...
| Sprint 6 (Onboarding) | 🔄 | ~97% | Wizard + flujos; ✅ [6g]–[6h]; ✅ [6i] CTA Nueva materia + onboarding card + mensaje post-wizard; ✅ [6j] `session-card` (pre-clase + tips + `source_date`), filtro tipo en sesiones, calendario `Pre-clase` vs R0. ✅ Fix: wizard último paso sin bloqueo → doble clic creaba materias duplicadas (`isCompleting` + ref + navegación inmediata). ✅ **SA-94 / fix-notif-batch:** wizard bulk una sola notificación/email de resumen (`skip_sessions_created_notification` en `createTopic` + resumen en `onboarding-wizard`). ✅ **SA-95 / fix-notif-followup:** `sendUpdates: 'none'` en `GoogleCalendarService`; además `skip_google_calendar_sync` en bulk `createTopic` + un solo `syncSessions` al cerrar wizard (cursada/libre) para no disparar N sync por tema. ✅ **SA-96 / fix-preclass-overdue:** `generatePreClassSessions` con `generatedAt` + día UTC de agenda (mismo día que la clase si el “día anterior” cae antes de `generatedAt`); sin insertar en el pasado ni solapar inicio de cursada. ✅ **SA-97:** callback Google (`returnTo=onboarding`) ya no setea `onboarding_completed` ni redirige al dashboard; vuelve a `/onboarding?google_connected=true`. ✅ **Seguimiento SA-97:** al volver con `google_connected=true`, el wizard avanza solo al paso Materia con disponibilidad por defecto (tres turnos) para que `saveOnboardingAvailability` corra al cerrar. ✅ **SA-98 / fix-exam-date-dmy:** fechas DD/MM del PDF ya no rompen el insert en Postgres (`normalizeExamDateToIso`, `createExamSchema`/`updateExamSchema`, `parseParcialDateUtcMidnight`, pre-carga parciales en wizard). ✅ **SA-99 / fix-ia-exam-dates:** `mergeExamsWithScheduleDates` en `extraction-processor` fusiona fechas ISO de entradas EXAM/RECOVERY del `normalizedSchedule` en `exams` (match por unidades/tipo/texto + síntesis si el modelo no devolvió exams). ✅ **SA-100 / fix-onboarding-cursada:** `handleCursadaPath` ancla `source_date` a la última fecha de clase del rango si hay más temas que días (evita fallback silencioso a “hoy”); pre-clase usa `isSlotOnClassDay` (mismo día UTC o mismo día civil AR); paso Distribución bloquea al avanzar si el `topic-distributor` reporta falta de clases/minutos. ✅ **SA-101 / fix-pdf-topic-budget:** orden wizard CURSANDO = cursada básica (horario + parciales, sin temas) → PDF → temas/asignación (horario solo lectura) → distribución; `processPDF(extractionId, cursadaContext)` arma `TopicGroupingInput` con clases hasta 1er parcial + `groupTopicsWithAI` (fallback `groupTopics`). **Próximo:** smoke manual / E2E cursada opcional. |
| Sprint 7 (IA PDFs) | ✅ | 100% | Upload, strategy IA, extracción, UI revisión, actions, onboarding+metadata; cierre **[7i]** `buildSubjectWizardInput` (paridad Nueva materia, PDF → `pdfMetadata`). ✅ **SA-102 [7z]:** `ai_extractions.error_detail` + `grouping_used_ai`; `AI_FALLBACK_MODEL` (reintento mismo proveedor); `errorDetail` en proveedores OpenAI/Google. Migración `20260409120000_ai_extractions_error_detail.sql`. Ver `NEW_SPRINTS_PLAN`. ✅ **SA-104 [7z-pdf-timeout-race]:** timeout de `Promise.race` en `processPDF` = `AI_TIMEOUT_MS` o `2×` si hay fallback de modelo distinto; override `AI_EXTRACTION_RACE_MS`. ✅ **SA-105 [7z-topic-grouper-obs]:** observabilidad `TopicGrouper` — INFO progreso unidad pesada i/N, WARN con `durationMs`/error/model/`errorDetail` truncado, motivo de parse vacío (`emptyReason`), INFO fallback determinista; opcional `AI_TOPIC_GROUPER_DEBUG`. ✅ **SA-106 [7z-topic-grouper-mime]:** Gemini acepta `application/json`/`text/plain` para agrupación IA — payload como partes de texto (`google.ts`); OpenAI paridad `type: text` en lugar de pseudo-PDF. |
...
**Estado actual (2026-04-08):** Sprint 1–4 ✅, **Refactoring Arquitectural ✅**, **Sprint 5 The Curator ✅**, Sprint 6 onboarding 🔄 (~97%), **Sprint 7 IA PDFs ✅**, Sprint 8 ✅, **Sprint 9 Gamificación ✅**  
