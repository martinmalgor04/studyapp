...
export default function AvailabilityPage() {
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-center text-on-surface-variant">Cargando disponibilidad...</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-on-surface">Disponibilidad de Estudio</h1>
      ...
      {viewMode === 'calendar' && success && (
        <div className="mb-4 rounded-md bg-secondary-container/30 p-4">
          <p className="text-sm text-on-secondary-container">Disponibilidad guardada correctamente</p>
        </div>
      )}
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2">
        ...
        <button
          ...
          className={`rounded-xl border-2 p-6 transition-all text-left ${
            configMethod === 'manual'
              ? 'border-outline bg-surface-container-low shadow-md'
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-lg font-semibold text-on-surface">Configuración Manual</h3>
          </div>
          <p className="text-sm text-on-surface-variant">Definir horarios manualmente</p>
        </button>
        ...
        <button
          ...
          className={`rounded-xl border-2 p-6 transition-all text-left relative ${
            configMethod === 'google'
              ? 'border-tertiary bg-tertiary-container/30 shadow-md'
              : 'border-outline-variant bg-surface-container-lowest hover:border-tertiary/50'
          }`}
        >
          <div className="absolute -top-2 -right-2 bg-tertiary text-on-tertiary text-xs font-bold px-2 py-0.5 rounded-full">
            ⭐ Recomendado
          </div>
          <div className="flex items-center gap-3 mb-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-tertiary-container/30">
              <svg className="h-5 w-5 text-tertiary" fill="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-sm text-on-surface-variant">Sincronización automática</p>
        </button>
      </div>

      {/* Google Calendar Section */}
      {configMethod === 'google' && (
        <div className="mb-6 rounded-lg border-2 border-tertiary/30 bg-tertiary-container/30 p-6">
          {isGoogleConnected ? (
            <>
              <div className="flex items-center gap-2 mb-4">
                <svg className="h-5 w-5 text-on-secondary-container" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span className="font-medium text-on-surface">Google Calendar conectado</span>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={saving}
                  className="rounded-md bg-tertiary px-4 py-2 text-sm font-medium text-on-tertiary hover:bg-tertiary-dim disabled:opacity-50"
                >
            ...
            <>
              <h3 className="font-medium text-on-surface mb-2">Conectar Google Calendar</h3>
              <p className="text-sm text-on-surface-variant mb-4">
      ...
      {configMethod === 'manual' && (
        <>
            ...
            <div className="flex rounded-md bg-surface-container p-1">
              <button
                ...
                className={`rounded px-3 py-1.5 text-sm font-medium transition-colors ${
                  viewMode === 'calendar' ? 'bg-surface-container-lowest text-on-surface shadow-sm' : 'text-on-surface-variant hover:text-on-surface'
                }`}
              >
                Calendario
              </button>
              <button
                ...
                className={`rounded px-3 py-1.5 text-sm font-medium transition-colors ${
                  viewMode === 'list' ? 'bg-surface-container-lowest text-on-surface shadow-sm' : 'text-on-surface-variant hover:text-on-surface'
                }`}
              >
                Lista
              </button>
          ...
          <div className="bg-tertiary-container/30 border border-tertiary/30 rounded-lg p-4 mb-6">
        <div className="flex gap-3">
          ...
          <div>
            <h3 className="font-medium text-tertiary">Cómo funciona</h3>
            ...
            <>
              ...
              <div className="mt-6 flex justify-end">
                <button
                  ...
                  disabled={saving}
                  className="rounded-md bg-tertiary px-6 py-2.5 text-sm font-medium text-on-tertiary shadow-sm hover:bg-tertiary-dim disabled:opacity-50"
                >
